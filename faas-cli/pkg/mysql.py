...
if __name__ == "__main__":
    database = Database('localhost','root','123456','openfaas_test')
    database.create_model_table()


    # database.create_image_table()
    # for i in range(10):
    #     with open("./images/"+str(i+1)+".jpg", "rb") as file:
    #         image = file.read()
    #     database.insert_image(str(i+1), image)

    # database.update_label('1', 6)

    model = database.get_model("cnn_mnist")
    print(model)